...
//Compile: g++ -std=c++11 linear.cpp -o linear
//=======================================================
...
//Function: linear_hash
//Description: Attempts to hash value useing linear probing
//Returns: An index for the arr array.
int linear_hash(long long int val, int i)
{
  //If the key value is positive use normal linear-hashing 
  //process, otherwise, masking needs to occur for negative long-long int values:
  if(val >= 0)
  {
    int hash;
    hash = (val + i) % 1000000;
...
void insert(long long int key, int keys)
{
  ...
  while(arr[hashVal] != 0)
  {
    ...
    i++;
    hashVal = linear_hash(key, i); //Wrap-around if necessary
  }
...
int main()
{
  ...
  //Selection from user:
  cout << "======================================="<< endl;
  cout << "Welcome to the linear probing program! " << endl;
  cout << "=======================================" << endl;
